...
async def test_lambda_cold_start(
    function_name: str, num_tests: int = 10
) -> Dict[str, Any]:
    ...
    async with session.client("lambda", region_name="us-east-1") as lambda_client:
        ...
        for i in range(num_tests):
            ...
            response = await lambda_client.invoke(
                FunctionName=function_name, Payload=json.dumps(test_payload)
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python benchmark_cold_start.py AmiraLambdaParallelStack-ProcessingFunction"
        )
        sys.exit(1)
    ...
    print(
        f"   Concurrent throughput: {concurrent_results['throughput']:.1f} invocations/second"
    )
    print(
