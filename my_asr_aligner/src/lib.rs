...
fn is_error(item_name: &str, hyp_phoneme: &str) -> bool {
...
#[pyfunction(signature = (expected_items, ref_phons, hyp_phons, confidences))]
#[pyo3(text_signature = "(expected_items, ref_phons, hyp_phons, confidences)")]
fn word_level_alignment(
    ...
    let mut word_alignment_result: Vec<String> = Vec::new();
    let mut errors: Vec<bool> = Vec::new();
    let mut matched_confidence: Vec<f32> = Vec::new();

    let mut ri: usize = 0;
    let mut hj: usize = 0;
    let mut pending_ref_indices: Vec<usize> = Vec::new();

    for op in diff.ops() {
        for change in diff.iter_changes(op) {
            match change.tag() {
                ChangeTag::Equal => {
                    if !pending_ref_indices.is_empty() {
                        for _idx in pending_ref_indices.drain(..) {
                            word_alignment_result.push("-".to_string());
                            errors.push(true);
                            matched_confidence.push(0.0);
                        }
                    }
                    if hj < hyp_phons.len() {
                        word_alignment_result.push(hyp_phons[hj].clone());
                        errors.push(false);
                        let conf = if hj < confidences.len() { confidences[hj] } else { 0.0 };
                        matched_confidence.push(conf);
                    }
                    ri += 1;
                    hj += 1;
                }
                ChangeTag::Delete => {
                    pending_ref_indices.push(ri);
                    ri += 1;
                }
                ChangeTag::Insert => {
                    if let Some(ref_idx) = pending_ref_indices.first().cloned() {
                        pending_ref_indices.remove(0);
                        ...
                        let is_err = is_error(ref_word, hyp_word);
                        word_alignment_result.push(if is_err {
                            "-".to_string()
                        } else {
                            hyp_word.to_string()
                        });
