...
class TestTranscribePhrase:
    ...
    def test_successful_transcription(
        self, mock_slice_audio, sample_activity, mock_kaldi_client, mock_w2v_client
    ):
        ...
        mock_kaldi_client.transcribe.assert_called_once_with(
            sample_activity, 0, mock_audio_data
        )
        mock_w2v_client.transcribe.assert_called_once_with(
            sample_activity, 0, mock_audio_data
        )
...
class TestAlignPhraseTranscripts:
    ...
    @patch(
        "src.orf_rescoring_pipeline.rules.rescoring.get_word_level_transcript_alignment_w2v"
    )
    def test_successful_alignment(self, mock_w2v_align, mock_kaldi_align):
        ...
        kaldi_matches, w2v_matches = align_phrase_transcripts(
            story_phrase=story_phrase, kaldi_transcript=kaldi_transcript, w2v_transcript=w2v_transcript
        )
        ...
        mock_kaldi_align.assert_called_once_with(story_phrase=story_phrase, transcript_text=kaldi_transcript)
        mock_w2v_align.assert_called_once_with(story_phrase=story_phrase, transcript_text=w2v_transcript)

    @patch("src.orf_rescoring_pipeline.rules.rescoring.get_word_level_transcript_alignment")
    @patch(
        "src.orf_rescoring_pipeline.rules.rescoring.get_word_level_transcript_alignment_w2v"
    )
    def test_empty_transcripts(self, mock_w2v_align, mock_kaldi_align):
    ...
    def test_alignment_exceptions(self, mock_w2v_align, mock_kaldi_align):
        ...
        with pytest.raises(Exception, match="Kaldi alignment failed"):
            align_phrase_transcripts(story_phrase=story_phrase, kaldi_transcript=kaldi_transcript, w2v_transcript=w2v_transcript)
...
class TestRescorePhraseErrors:
    ...
    def test_basic_rescoring_logic(self):
        """Test basic error rescoring logic."""
        deepgram_matches = [1, 0, 1, 0, 1]
        kaldi_matches = [0, 1, 1, 0, 0]
        w2v_matches = [1, 0, 0, 1, 1]

        error_array = rescore_phrase_errors(deepgram_matches=deepgram_matches, kaldi_matches=kaldi_matches, w2v_matches=w2v_matches
        )
    ...
    def test_all_systems_agree_correct(self):
    ...
    def test_all_systems_agree_incorrect(self):
...
class TestRescorePhrase:
    ...
    def test_successful_rescoring(
        ...
        result = rescore_phrase(activity=sample_activity, feature=feature, kaldi=mock_kaldi_client, w2v=mock_w2v_client
        )
        ...
        mock_transcribe.assert_called_once_with(
            activity=sample_activity, phrase_index=feature.phrase_index, kaldi=mock_kaldi_client, w2v=mock_w2v_client
        )
        mock_align.assert_called_once_with(
            story_phrase="hello world test", kaldi_transcript="kaldi transcript", w2v_transcript="w2v transcript"
        )
        mock_rescore_errors.assert_called_once_with(deepgram_matches=[1, 0, 1], kaldi_matches=[1, 0, 1], w2v_matches=[1, 1, 0])
    ...
    def test_transcription_failure_propagation(
        ...
        with pytest.raises(Exception, match="Transcription failed"):
            rescore_phrase(activity=sample_activity, feature=feature, kaldi=mock_kaldi_client, w2v=mock_w2v_client)
