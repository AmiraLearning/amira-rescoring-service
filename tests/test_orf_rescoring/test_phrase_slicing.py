...
class TestLoadActivityAudioDataFromS3:
    ...
    def test_successful_audio_loading(
        self, mock_s3_addr, mock_get_client, sample_activity, mock_audio_data
    ):
        """Test successful loading of audio data from S3."""
        # Setup mocks
        mock_s3_addr.return_value = Mock(
            bucket="test-bucket", key="test_activity_123/complete.wav"
        )
        mock_s3_client = Mock()
        ...
        # Test the function
        load_activity_audio_data_from_s3(activity=sample_activity, audio_bucket="test-bucket")
        ...
        mock_s3_addr.assert_called_once_with(
            "s3://test-bucket/test_activity_123/complete.wav"
        )
    ...
    def test_s3_download_failure(self, mock_s3_addr, mock_get_client, sample_activity):
    ...
    def test_file_read_failure(
        self, mock_open, mock_s3_addr, mock_get_client, sample_activity
    ):
    ...
    def test_correct_s3_uri_construction(self, sample_activity):
        ...
        with patch(
            ...
            with patch(
                ...
                try:
                    load_activity_audio_data_from_s3(activity=sample_activity, audio_bucket="my-audio-bucket")
                except Exception:
                    pass  # We expect an exception

                # Verify correct URI was constructed
                expected_uri = (
                    f"s3://my-audio-bucket/{sample_activity.activity_id}/complete.wav"
                )
...
class TestSliceAudioFileData:
    ...
    def test_successful_audio_slicing(
        self, mock_from_file, sample_activity, mock_audio_data
    ):
        ...
        audio_data, (start_ms, end_ms) = slice_audio_file_data(activity=
            sample_activity, phrase_index=phrase_index
        )
    ...
    def test_no_audio_file_data(self, sample_activity):
        """Test slicing when activity has no audio file data."""
        sample_activity.audio_file_data = None

        audio_data, (start_ms, end_ms) = slice_audio_file_data(activity=sample_activity, phrase_index=0)

        assert audio_data is None
        assert start_ms is None
        assert end_ms is None

    def test_phrase_not_in_any_page(self, sample_activity, mock_audio_data):
        ...
        audio_data, (start_ms, end_ms) = slice_audio_file_data(activity=
            sample_activity, phrase_index=non_existent_phrase_index
        )
    ...
    @patch("src.orf_rescoring_pipeline.alignment.audio_processing.AudioSegment.from_file")
    def test_phrase_not_aligned(self, mock_from_file, sample_activity, mock_audio_data):
    ...
    def test_multiple_pages_phrase_lookup(self, sample_activity, mock_audio_data):
...
class TestAudioProcessingEdgeCases:
    ...
    @patch("src.orf_rescoring_pipeline.alignment.audio_processing.AudioSegment.from_file")
    def test_negative_timing(self, mock_from_file, sample_activity, mock_audio_data):
    ...
    def test_empty_pages_list(self, sample_activity, mock_audio_data):
